...
async def update_product(id:int, product: schemas.ProductCreate, db: Session = Depends(get_db)):
...
async def update_order_status(id: int, status: str, db: Session = Depends(get_db)):
    return await crud.update_order_status(db, id, status)